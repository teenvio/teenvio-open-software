...
class UTLIni{
	 ...
	 * @throws TeException
	 */
	public static function toTmpTable(BDBase $BDBase,$namespace="GENERAL",$tableName="",$structure=null,$exclude=null){
		if (is_null($BDBase->BD2)){
			throw new TeException('La BDBase pasada no tiene instanciado un plan',__LINE__,__CLASS__);
		}
		...
		if (!$BDBase->BD2->query($SQL)){
			throw new TeException('Fallo en el sql al truncar la tabla temporal: '.$BDBase->BD2->ultimo_error, __LINE__,__CLASS__);
		}
		
		$SQL="";
		if ($tableName=="") $tableName=$namespace;
		
		foreach (UTLIni::$conf[$namespace] as $clave=>$campos){
			if (!is_array($campos) || count($campos)==0) continue;
			...
			if ($SQL==""){
				...
				if (!$BDBase->BD2->query($SQL)){
					throw new TeException('Fallo en el sql al construir la tabla temporal: '.$BDBase->BD2->ultimo_error, __LINE__,__CLASS__);
				}
			}
			
			if (!$BDBase->BD2->InsertTabla($tableName, null, null, $campos)){
				throw new TeException('Fallo al insertar un registro en la tabla temporal: '.$BDBase->BD2->ultimo_error, __LINE__,__CLASS__);
			}
		}
		return true;
	}
